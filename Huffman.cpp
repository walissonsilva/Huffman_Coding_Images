...
ofstream saida("Arquivos/encoded.txt");
 
// A Huffman tree node
struct MinHeapNode
{
    unsigned long long int data;                // One of the input characters
    unsigned long long int freq;             // Frequency of the character
    MinHeapNode *left, *right; // Left and right child
 
    MinHeapNode(unsigned long long int data, unsigned long long int freq)
    {
        left = right = NULL;
        this->data = data;
        this->freq = freq;
...
int main(){
    ifstream arquivo("Arquivos/IMG.txt");
    stringstream ss;

    int arr[256], freq[256];
    string line;
    int pixel, f;
